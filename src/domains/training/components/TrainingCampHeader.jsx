...
export default function TrainingCampHeader({ event, phase, levelLabel, sessionNo, eventType }) {
  const { t } = useLanguage()
  const isFixture = eventType === EVENT_TYPES.FIXTURE_GENERATION
  const level = getTrainingLevel(event?.level)
  const name = event?.name || ''

  // Personalidad del evento (BUILD-TC-003/004): cada estado habla como la sesión.
  const pill = isFixture
    ? ({
    ...
    <header className={styles.banner}>
      <div className={styles.bannerTop}>
        <span className={styles.bannerEyebrow}>
          {isFixture
            ? (sessionNo ? t('training.fixtureTag', { no: sessionNo }) : t('training.fixtureEyebrow'))
      ...
      <div className={styles.bannerTitle}>
        <span className={styles.bannerTitleIcon}>{isFixture ? '📋' : level.icon}</span>
        <span>{name}</span>
      </div>
      <div className={styles.bannerMeta}>
        {isFixture ? (
          <span className={`${styles.chip} ${styles.chipLevel}`}>{t('training.fixtureBadge')}</span>
        ) : (
